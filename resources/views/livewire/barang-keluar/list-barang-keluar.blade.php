<div class="absolute w-[81%]">
    ...
    <div class=" py-2 px-2 my-2 h-23 rounded-md h-[35rem] bg-gray-100">
        ...
        <div class=" flex justify-between items-center">
            ...
            {{-- Button Export --}}
            <div>
                @livewire('BarangKeluar.LaporanBarangKeluar')
            </div>
            {{-- end button Export --}}
            {{-- paginate --}}
            ...
            <table class="min-w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600 ">
                        <th scope="row" class="p-4">{{$item->id_barang_keluar}}</th>
                        ...
                        <td scope="row" class="p-4"> {{$item->jumlah_keluar}} {{$item->unit->name}}
                        </td>
                        <td scope="row" class="p-4">{{$item->keterangan}}</td>
                        <td class="py-4 border-b text-center flex">
                            <a href="{{route('editBarangMasuk', $item->id_barang_keluar)}}"
